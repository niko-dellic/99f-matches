...
const jsonData = JSON.parse(
  fs.readFileSync("./aliens_second_round_final.json", "utf8")
);
...
// siubtract 223 from alien id
jsonData.aliens.map((alien, index) => {
  alien.id = parseInt(alien.id) - 223;
});
...
jsonData.aliens.map((alien, index) => {
  ...
  alien.githubPath = `https://github.com/niko-dellic/99f-matches/blob/main/img-r2/${imageNum}.${imgType}?raw=true`;
});
...
(async () => {
  ...
  // await page.waitForSelector("h1.your-name")
    ...
    await page.screenshot({
      path: `./output2/output_${plot.indexOf(dataItem) + start}.png`,
      fullPage: true,
    });
